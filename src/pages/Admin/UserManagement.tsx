...
import { useGetAllUsersQuery, useUpdateProfileMutation } from "@/redux/features/auth/auth.api";
// import { Switch } from "@/components/ui/switch";
import { Phone, MapPin, User as UserIcon, Shield, Ban, CheckCircle, XCircle, Save, X } from "lucide-react";
...
const roleColors: Record<UserRoleType, string> = {
  [role.superAdmin]: "bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-300",
  [role.admin]: "bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300",
  [role.sender]: "bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300",
  [role.receiver]: "bg-orange-100 text-orange-800 dark:bg-orange-900/30 dark:text-orange-300",
  superAdmin: "",
  admin: "",
  receiver: "",
  sender: ""
...
const statusColors: Record<IsActiveType, string> = {
  [IsActive.ACTIVE]: "bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-300",
  [IsActive.INACTIVE]: "bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-300",
  [IsActive.BLOCKED]: "bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300",
  ACTIVE: "",
  INACTIVE: "",
  BLOCKED: ""
};
...
export default function UserManagement() {
  ...
  // const handleBlockUser = async (userId: string, currentStatus: IsActiveType) => {
  //   try {
  //     const newStatus = currentStatus === IsActive.ACTIVE ? IsActive.BLOCKED : IsActive.ACTIVE;
  //     await updateProfile({
  //       userId,
  //       formData: { isActive: newStatus }
  //     }).unwrap();
      
  //     toast.success(`User ${newStatus === IsActive.ACTIVE ? 'unblocked' : 'blocked'} successfully`);
  //     refetch();
  //   } catch (error) {
                    ...
                    <TableRow key={user._id} className="hover:bg-muted/30">
                      ...
                      <TableCell>
                        {editingUser === user._id ? (
                          <Select value={updatedRole} onValueChange={(value) => setUpdatedRole(value as UserRoleType)}>
                            <SelectTrigger className="w-[140px]">
                              <SelectValue placeholder="Select role" />
                            </SelectTrigger>
                            <SelectContent>
                              {getUserRoleValues()
                                .filter(roleValue => roleValue !== role.superAdmin) // Don't allow changing to super admin
                                .map((roleValue) => (
                                <SelectItem key={roleValue} value={roleValue}>
                                  {roleValue.replace("_", " ")}
                                </SelectItem>
                      ...
                      <TableCell>
                        {editingUser === user._id ? (
                          <Select value={updatedStatus} onValueChange={(value) => setUpdatedStatus(value as IsActiveType)}>
                            <SelectTrigger className="w-[140px]">
                          ...
                          <div className="flex items-center justify-end gap-4">
                            {/* <div className="flex items-center gap-2">
                              <span className="text-sm text-muted-foreground">
                                {user.isActive === IsActive.ACTIVE ? "Block" : "Unblock"}
                              </span>
                              <Switch
                                checked={user.isActive === IsActive.ACTIVE}
                                onCheckedChange={() => handleBlockUser(user._id, user.isActive)}
